...
export default function ProductsCatalog() {
  ...
  const filtered = useMemo(() => {
    return allProducts.filter((p) => {
      ...
      const fiberOk = fibers.length === 0 || fibers.includes(p.fiber)
      const colorOk = colors.length === 0 || colors.includes(p.color)
      const densOk = dens.length === 0 || dens.includes(p.density)
      const hOk = p.height >= minH && p.height <= maxH
      return catOk && fiberOk && colorOk && densOk && hOk
    })
  }, [cats, fibers, colors, dens, minH, maxH])

  // درصدهای پرشدگی اسلایدر
      ...
      <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)]">
        ...
        >
          ...
          <FilterBlock title="نوع الیاف">
            <ul className="space-y-3 pt-1">
              {fiberTypes.map((f) => {
                const checked = fibers.includes(f)
                return (
                  <li key={f}>
                    <button
                      onClick={() => toggle(setFibers, fibers, f)}
                      className="flex w-full items-center justify-between text-sm text-slate-600 transition hover:text-brand-600"
                    >
                      <span>{f}</span>
                      <span
        ...
        <div>
          ...
          <div className="glass mb-8 flex flex-col items-center gap-4 rounded-2xl px-5 py-4 sm:flex-row sm:justify-between">
            ...
            <p className="text-sm text-slate-500">
              ...
              <span className="font-bold text-slate-700">۲۴</span> محصول
            </p>
          </div>
